...
function vw_storefront_setup() {
	...
	if (is_admin() && ('themes.php' == $pagenow) && isset($_GET['activated']) && wp_get_theme()->get('TextDomain') === 'vw-storefront') {
		wp_redirect(admin_url('themes.php?page=vw_storefront_guide'));
	}
}
endif;
...
function vw_storefront_scripts() {	
	...
	wp_enqueue_script( 'vw-storefront-jquery-wow', esc_url(get_template_directory_uri()) . '/assets/js/wow.js', array('jquery') );
	wp_enqueue_style( 'vw-storefront-animate-css', esc_url(get_template_directory_uri()).'/assets/css/animate.css' );
...
function vw_storefront_tagline_hide_show(){
	if(get_theme_mod('vw_storefront_tagline_hide_show') == '1' ) {
		return true;
	}
	return false;
}
